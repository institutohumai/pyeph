...
class Basket(Getter):
	...
	def get_latest_basket_date(self):
		...
		try:
			...
			logger.info(f"Última canasta disponible (mirror github): {latest_date.strftime('%Y-%m')}")
			return latest_date
	...
	def _download_from_github_mirror(self):
		...
		for f in self.make_filenames(year_month):
			...
			df_inicial = pd.concat([df_inicial, df_f])
		return df_inicial, year_month

	def get_df(self, inform_user=True):
		"""
			Retorna el DataFrame con la serie historica de canastas (CBA y CBT)
			por region.

			Se prioriza la fuente oficial (datos.gob.ar - SSPM), que se actualiza
			mensualmente. Si esa fuente no esta disponible se cae al mirror estatico
			en github pages.
		...
		"""
		try:
			df_inicial = self._download_from_datos_gob()
			latest = pd.to_datetime(df_inicial['indice_tiempo']).max()
			if inform_user:
				logger.info(
					"CBT y CBA obtenidas de datos.gob.ar. Último mes disponible: {}".format(
						latest.strftime('%Y-%m')
					)
				)
		except (requests.exceptions.RequestException, ValueError, pd.errors.ParserError) as e:
			logger.warning(
				"No se pudo obtener las canastas desde datos.gob.ar ({}). "
				"Usando el mirror estatico en github pages.".format(e)
			)
			df_inicial, year_month = self._download_from_github_mirror()
			if inform_user:
				logger.info(
					"CBT y CBA mas actualizada que se obtuvo: {}".format(
